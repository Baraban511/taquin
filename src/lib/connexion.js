...
export async function connexion(login, password) {
    ...
    return await fetch('https://api.ecoledirecte.com/v3/login.awp?v=3', {
        ...
        if (data.code === 200) {
            throw new Error("Votre compte EcoleDirecte ne prend pas en charge EDC pour l'instant")
        }
        if (data.code === 250) {
            var link = {
...
export async function doubleauth(token) {
    return await fetch('https://api.ecoledirecte.com/v3/connexion/doubleauth.awp?verbe=get', {
        method: 'POST',
        headers: {
            "User-Agent":
                "Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:109.0) Gecko/20100101 Firefox/119.0",
